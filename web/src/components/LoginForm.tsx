    ...
    <div className="w-full max-w-sm rounded-xl border border-[var(--border)] bg-[var(--surface)] p-8">
      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-4">
        <label className="flex flex-col gap-1 text-[13px]">
          <span className="text-[var(--text2)]">Username or email</span>
          <input
            value={username}
            onChange={(e) => setUsername(e.target.value)}
            autoComplete="username"
            className="rounded-md border border-[var(--border)] bg-[var(--bg2)] px-3 py-2 text-[14px] focus:border-[var(--accent)]"
          />
        </label>
        ...
        {error && (
          <div role="alert" className="rounded-md border border-[var(--err)]/40 bg-[var(--err)]/10 px-3 py-2 text-[13px] text-[var(--err)]">
            {error}
          </div>
        )}
