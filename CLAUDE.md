...
- **Inference backend seam (`Generator`):** the agentic `Loop` submits through a consumer-defined `inference.Generator` interface. Two implementations: `*infra.BatchScheduler` (gRPC Python worker — the self-written `transformers` engine, `inference.mode=worker`, default) and `*infra.OpenAIClient` (`internal/infrastructure/inference/openai_client.go`) that calls an OpenAI-compatible upstream (vLLM, llama-server) directly over HTTP/SSE and maps chunks → engine events. `inference.mode=openai` + `AI_FACTORY_INFERENCE_URL`/`_MODEL` selects it. The direct path **bypasses the Python worker** (no gRPC hop) — the production gateway→vLLM pattern; `mode=worker` keeps the self-written engine on the path.
...
- **Inference backend (`inference.mode`):** `worker` (default) routes generation through the gRPC Python worker (`BatchScheduler`) — the path that serves the self-written tokenizer/sampling/KV/forward engine. `openai` makes the Go server call an OpenAI-compatible upstream directly (`OpenAIClient`, HTTP/SSE) — vLLM/llama-server — with no worker hop and no gRPC. Both satisfy the consumer-defined `inference.Generator` seam and emit the same event stream, so the agentic loop, tools, usage and billing are unchanged. Requires `AI_FACTORY_INFERENCE_URL` + `_MODEL` (model must match the upstream's served name). This is the gateway→vLLM pattern used in the Kubernetes setup.
...
- **Quota enforcement (2026-09-18 ✅):** `usage.QuotaEnforcer` (`internal/services/usage/quota_enforcer.go`) reads `tenant_quotas` (which had CRUD but no enforcement) plus the `usage_daily` rollup and blocks `/v1/chat/completions` when current-period usage `used >= limit` → `429 QUOTA_EXCEEDED`. `quota_type` is matched by prefix (`tokens…`/`requests…`) and `period` (`daily`/`monthly`) defines the UTC window; unknown types (`concurrent_requests`, `gpu_hours`) and `limit_value <= 0` are ignored. Wired through the consumer-defined `inference.QuotaGate` port (adapter in `internal/app/adapters.go`); the handler checks it after deployment resolution, before billing/session. Infra errors fail open (like the rate limiter). Config `quota.mode` = `off|shadow|enforce` (env `AI_FACTORY_QUOTA_MODE`, default **`shadow`**); metric `quota_exceeded_total{tenant,quota_type,mode}`. Reads come from the rollup, so enforcement lags ≤ one Roller interval (5 s).
...
| `--engine vllm` | VLLMBackend | Qwen2.5-1.5B-Instruct (`--vllm-model`) | vLLM OpenAI server — spawn `vllm serve` locally, or `--vllm-url` for a running/K8s server |

`LlamaBackend` and `VLLMBackend` share the OpenAI-compatible plumbing in `worker/engines/openai_compat.py` (`OpenAICompatClient` + `OpenAICompatBackend`). vLLM needs no Python dependency: it is only spawned as a subprocess (or reached over HTTP). Details: `docs/superpowers/specs/2026-08-10-qwen35-gguf-engine-design.md`.
...
- **Not yet:** persistence for other domains (some worker-only state such as workload refs is dev-only). Tool sandboxing exists but is opt-in (`tools.executor=docker`; the default `local` executor runs `run_command` unsandboxed).
