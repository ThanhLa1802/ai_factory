...
- **Tool Executor**: Interface trong Go (`ToolExecutor`) định nghĩa cách execute tool (`Execute`/`ListTools`/`CanExecute`). `LocalToolExecutor` chạy trực tiếp trên host (timeout 30s); `DockerToolExecutor` chạy trong container dùng-một-lần (sandbox, chọn bằng `tools.executor=docker`). Có 4 built-in tools: `read_file`, `write_file`, `run_command`, `list_files`.
