...
class LlamaServer:
    def __init__(self, gguf, host="127.0.0.1", port=8081, bin="llama-server",
        ...
        self.host = host
        self.port = port
        self.bin = bin  # giữ lại cho error message khi không spawn được
        self.proc = None
        self._log_path = None
        self._log_file = None
        self.cmd = [
    ...
    def start(self, timeout=120.0):
        ...
        # Redirect output tới log file (KHÔNG dùng PIPE — Windows pipe buffer ~4KB
        # sẽ đầy bởi banner llama-server → child block → start() timeout dù binary OK).
        # Log file giữ stderr để troubleshoot ở Task 8 E2E.
